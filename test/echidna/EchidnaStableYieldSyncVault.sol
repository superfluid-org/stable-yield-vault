...
contract EchidnaStableYieldSyncVault is EchidnaVaultHarnessBase {
    ...
    constructor() {
        ...
        // The flat participation fee is now paid in the underlying asset (no ETH needed); actor USDC
        // funding covers it. Give actors ETH anyway as a harmless harness convenience.
    ...
    function deposit(uint8 actorIdx, uint96 amount) external keepAlive {
        ...
        try _vault.deposit(amt, actor) returns (uint256 shares) {
            _ghostSupply += shares;
            // C.1: a non-dust deposit grants the receiver yield units at deposit time, on the net
            // principal (gross minus the flat fee).
            if (amt > fee && amt - fee >= _fundManager.RAW_PER_UNIT()) {
