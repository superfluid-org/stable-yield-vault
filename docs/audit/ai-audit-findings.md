...
**Description**
For underlyings with decimals > 6 (e.g., DAI: `RAW_PER_UNIT = 1e12`), `_toUnit(assets) = assets / 1e12` floors to **0** for any `assets < 1e12`, so a deposit of e.g. `5e11` wei mints `5e11` shares but grants `0` GDA pool units; subsequent `requestRedeem` enters `onRequestRedeem`, reads `userUnits = POOL.getUnits(owner) = 0`, and unconditionally reverts with `BAD_REDEEM_ARGS` — and shares are also non-transferable, so the holder's only recovery is to make a strictly larger second deposit and claim it (which they may not have liquidity for). For a holder whose entire balance is sub-unit deposits, the funds are permanently locked.
...
- **4-arg `redeem`/`withdraw` allow approved operators to redirect `receiver`** — `StableYieldAsyncVault.redeem / withdraw` — Code smells: only `_isOperator[controller][msg.sender]` checked, `receiver` is unconstrained — ERC-7540 standard permits this, but combined with the design's explicit non-transferability of shares it contradicts the soulbound-position invariant; a controller's broad operator approval becomes a full-position theft primitive if the operator key is compromised.
