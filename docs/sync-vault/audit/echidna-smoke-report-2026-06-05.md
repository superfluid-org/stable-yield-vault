...
| Property | Verdict |
|---|---|
| `redeem`, `withdraw` | **Harness artifact — invalid as a production bug** (Issue 1). Requires the FM super-token to sit at `availableBalance < 0`, a state Superfluid sentinels prevent; the harness models no liquidator. |
...
**Disposition.**
- **Downgrade Issue 1 to invalid / harness artifact.** No code change is justified by the echidna evidence as it stands.
- **Harness improvement:** either (a) add a sentinel actor (a handler that liquidates the FM's GDA stream when `availableBalance < 0`) so the run stays in production-reachable states, or (b) gate the `redeem`/`withdraw` F.2 asserts on FM solvency (`availableBalance >= 0`) so the artifact is excluded. Add a comment near the `warp_*` handlers noting the no-sentinel limitation.
- **Optional follow-up:** a dedicated test for the post-liquidation flow-restart path above, to decide whether a solvency-aware exit recalibrate (skip / `setStableYieldRate(0)`-style close when the reserve can't fund the restart) or a widened pause threshold (engage below the stream requirement, not only at `maxWithdraw(FM) == 0`) is warranted.
...
- **redeem / withdraw → harness artifact (invalid).** The brick requires the FM super-token at `availableBalance < 0`, which Superfluid sentinels prevent (they liquidate the stream at the zero-crossing). The harness models no liquidator, so it explores a state production never sustains. Empirically (boundary probe) the in-bounds exit works throughout the entire solvent region and only bricks once `availableBalance` is well below 0. **No code change is justified by this evidence.** The fix is on the *harness* (add a sentinel actor, or gate the F.2 asserts on solvency). A *separate, untested* path — post-liquidation flow-restart under a still-illiquid external — is noted in Issue 1 as a possible (narrow) follow-up, to be decided by its own test, not by this run.
